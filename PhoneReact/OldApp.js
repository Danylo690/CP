...
class App extends Component {
  ...
  async PickFiles() {
    try {
      ...
      pickedFiles.forEach(element => {
        ...
        this.fileSentOptions.NumberOfPacket.push(
  ...
  async SendFile(command) {
    ...
    switch (command) {
      ...
      case 'NeedName': {
        console.log(name);
        this.fileSentOptions.currentFileInBytes = base64.parse(
          await RNFS.readFile(this.fileSentOptions.path.pop(), 'base64'),
        );
        this.fileSentOptions.name.pop();
        this.fileSentOptions.currentLength =
          this.fileSentOptions.currentFileInBytes.length;
      ...
      case 'NeedNumberOfPackets': {
        ...
        this.state.client.write(
          this.fileSentOptions.NumberOfPacket.pop().toString(),
        );
        break;
      }
      case 'NeedPacket': {
        if (this.fileSentOptions.currentLength > bufferSize) {
          ...
          this.fileSentOptions.currentLength =
            this.fileSentOptions.currentLength - bufferSize;
        } else {
          start = this.fileSentOptions.currentNumberOfPacket * bufferSize;
        ...
        this.state.client.write(
          this.fileSentOptions.currentFileInBytes.slice(start, end),
        );
    ...
    return (
      <SafeAreaView>
        <Button
          title="Being client"
          color="#841584"
          onPress={() => this.toggleActionsShow('client')}
        />

        <Button
          title="Start ftp server"
          color="#841584"
          onPress={() => this.toggleActionsShow('server')}
        />

        {this.ShownActions === 'client' && (
          <>
            <TextInput
